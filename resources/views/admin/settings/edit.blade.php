...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.settings.update-setting', $setting) }}" method="POST" class="bg-white shadow rounded-lg">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="group"
                    ...
                    @foreach($groups as $grp)
                        <option value="{{ $grp }}" {{ old('group', $setting->group) === $grp ? 'selected' : '' }}>{{ ucfirst($grp) }}</option>
                    @endforeach
            ...
            <div>
                ...
                <textarea name="value"
                          id="value"
                          rows="3"
                          class="mt-1 shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md @error('value') border-red-500 @enderror">{{ old('value', $setting->value) }}</textarea>
                @error('value')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-end space-x-3 rounded-b-lg">
            <a href="{{ route('admin.settings.index', ['group' => $setting->group]) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                Cancel
            </a>
