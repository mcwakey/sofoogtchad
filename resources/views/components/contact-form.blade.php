...
<div {{ $attributes->merge(['class' => 'contact-form']) }}>
    ...
    @if(session('success'))
        <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div class="flex items-center">
                ...
                <span class="text-green-800">{{ session('success') ?: $successMessage }}</span>
            </div>
        </div>
    @endif
    ...
    <form action="{{ $action }}" method="{{ $method === 'GET' ? 'GET' : 'POST' }}" class="space-y-5">
        ...
        <div class="flex items-start">
            ...
            >
            <label for="privacy_consent" class="ml-2 text-sm text-gray-600">
                I agree to the <a href="{{ url('/privacy-policy') }}" class="text-green-600 hover:underline">Privacy Policy</a> and consent to being contacted regarding my inquiry.
