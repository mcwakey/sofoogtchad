        ...
        @foreach($aboutSections as $index => $section)
            ...
            >
                ...
                @if(isset($section['cta_text']) && isset($section['cta_url']))
                    <a href="{{ $section['cta_url'] }}" class="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-green-600 dark:bg-green-500 text-white font-semibold rounded-lg hover:bg-green-700 dark:hover:bg-green-600 transition-colors duration-200">
                        {{ $section['cta_text'] }}
        ...
        <section class="py-16 bg-gray-50 dark:bg-gray-800 transition-colors duration-200">
            <div class="container mx-auto px-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                    ...
                    @if(isset($mission) && $mission)
                        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg dark:shadow-gray-900/30 p-8 border-t-4 border-green-600">
                            ...
                            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">{{ $mission['title'] ?? 'Our Mission' }}</h3>
                    ...
                    @if(isset($vision) && $vision)
                        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg dark:shadow-gray-900/30 p-8 border-t-4 border-yellow-500">
                            ...
                            <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">{{ $vision['title'] ?? 'Our Vision' }}</h3>
                            <p class="text-gray-600 dark:text-gray-300 leading-relaxed">
                ...
                @if(isset($values) && is_array($values) && count($values) > 0)
                    <div class="mt-12">
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white text-center mb-8">{{ $valuesTitle ?? 'Our Core Values' }}</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
                            @foreach($values as $value)
                                <div class="text-center">
                                    ...
                                    <h4 class="font-semibold text-gray-900 dark:text-white">{{ is_array($value) ? ($value['title'] ?? $value['name'] ?? '') : $value }}</h4>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="container mx-auto px-4">
                <div class="text-center mb-12">
                    ...
                    @if(isset($teamDescription))
                        <p class="mt-4 text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">{{ $teamDescription }}</p>
                    @endif
                </div>
                    ...
                    @foreach($team as $member)
                        <div class="text-center group">
                            ...
                            @if(isset($member['position']) || isset($member['role']))
                                <p class="text-green-600 dark:text-green-400 font-medium">{{ $member['position'] ?? $member['role'] ?? '' }}</p>
                            @endif
                            @if(isset($member['bio']))
                                <p class="mt-2 text-gray-600 dark:text-gray-400 text-sm">{{ Str::limit($member['bio'], 100) }}</p>
                            @endif
                        </div>
                    @endforeach
                </div>
                            ...
                            <div class="relative flex items-start mb-8 last:mb-0 {{ $index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse' }}">
                                ...
                                <div class="ml-12 md:ml-0 md:w-5/12 {{ $index % 2 === 0 ? 'md:pr-12 md:text-right' : 'md:pl-12' }}">
                                    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-md dark:shadow-gray-900/30 p-6">
                                        ...
                                        @endif
                                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">{{ $milestone['title'] ?? '' }}</h3>
